...
当 Cowrite 口令同时给出页面 ID 与“原文精确片段”时，视为用户已明确选择 Method 5 和修改范围：

1. 先用 `cowrite_get_page` 读取最新正文与 revision，确认精确片段仍存在
2. 保持原意、事实、作者口吻和 Markdown 格式，只优化该片段
3. 默认目标是更凝练、口语化，去除冗余、套话和 AI 味
4. 用 `cowrite_update_page` 写回完整正文，页面其他内容一字不动
5. 必须传 `expected_revision`；冲突时重新读取并合并，绝不覆盖人工修改
