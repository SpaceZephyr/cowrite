...
- **Illustrate a complete Page** (pasted command from the Page-level 配图 button): MUST use bundled `article-batch-illustration` to plan a restrained 2-6 image set and bundled `image-studio` with GPT-Image-2 / LabNana to generate it. Keep one visual system across the set, upload only verified PNGs, re-read the latest page, then insert each result after its exact anchor from bottom to top with the latest revision. Never decorate every paragraph, silently switch models, or rewrite the article.
