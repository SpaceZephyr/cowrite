...
参考 `baoyu-slide-deck` 风格体系，为图片场景做了精选与扩展。
...
| 13 | **research-cover** ⭐固定预设 | 21:9 横版 + 左标题右图标卡片 + 米白深蓝 | 研究报告封面、长文头图、播客 banner、a16z 风 | 横版宽屏封面（2.35:1） |
...
| 横版宽屏封面 / 长文头图 / 研究报告 | research-cover | editorial-infographic / minimal |
...
> 四个 ⚠️ 节点**必须**获得用户显式确认。若 Cowrite 口令已经逐项列出“用户已确认”的类型、内容、风格与提示词模式，应直接执行，不要重复询问。
...
```
{cowrite_repo}/data/generated/image-studio/{YYYY-MM-DD}/
...
COWRITE_REPO="$(cd "$SKILL_DIR/../.." && pwd)"
if [ -f "$SKILL_DIR/.labnana.env" ]; then
...
python3 "$SKILL_DIR/scripts/generate_image.py" \
  ...
  --output "$COWRITE_REPO/data/generated/image-studio/{YYYY-MM-DD}/{file_name}.png" \
  --aspect-ratio "{aspect}" \
  --resolution "2K"
```
...
image-studio/
├── SKILL.md                          ← 你正在读
├── .labnana.env                      ← 可选的本地 API 配置（不进入 Git）
├── .labnana.env.example              ← 配置示例
├── scripts/
...
- API key 通过环境变量 `LABNANA_API_KEY` 或本地 `.labnana.env` 提供，禁止提交到 Git、泄漏到对话或日志
- Step 1–4 必须获得确认；Cowrite 口令已逐项声明“用户已确认”时视为确认完成
