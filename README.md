...
2. 点击顶部 `cowrite`，输入创作要求；Agent 会把当前页面全文作为上下文继续创作并安全写回。
...
6. Agent 读取页面最新 revision，调用指定 Skill 产出结果，再通过 MCP 精确写回。
7. 编辑器轮询更新，人和 Agent 可以继续编辑同一页面；revision 乐观锁会阻止相互覆盖。
